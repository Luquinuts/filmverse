...
- [x] 4.1 Migrar `src/app/feed/page.tsx`: store async (`getFeedReviews`, `getFollowedUsers`, `toggleFollow`, `searchProfiles`, `getSuggestedUsers`). Eliminar `ensureSeedData()` y `getSeedUsers()`. Agregar `getProfiles`, `searchProfiles`, `getSuggestedUsers` a store.ts.
- [x] 4.2 Modificar `src/app/api/ai/recommend/route.ts`: server obtiene reviews + watchlist de Supabase vía server client (`createClient` de `server.ts`) en vez de recibirlos del cliente. Backward compatibilidad con clients que envían datos.
- [x] 4.3 Modificar `src/app/api/ai/recommend/daily/route.ts`: POST genera recomendaciones + persiste en `public.recommendations` con `setDailyRecommendation`. GET devuelve desde DB con query param `userId`. Cache diario vía `unique(user_id, report_date)`.
- [x] 4.4 Crear `src/scripts/migrate-local-data.ts`: script one-time con `tsx`. Lee de JSON exportado via `--file`, escribe a Supabase con `service_role`. Idempotente: upsert por ID. Logea cada paso. Watchlist/listas/follows requieren userId (documentado).
- [x] 4.5 Eliminar `ensureSeedData()` del feed (`call` + import). Cubierto en 4.1 — sin rastros de `ensureSeedData`, `getSeedUsers`, `ReviewEntry` de local-store.
- [x] 4.6 `npx tsc --noEmit` final — compilación limpia sin errores. Smoke test manual post-merge en `/dashboard`, `/feed`, `/profile`, `/film/[id]`.
